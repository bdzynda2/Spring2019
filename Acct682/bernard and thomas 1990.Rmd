...
- Paper investigates possibility that stock prices fail to relfect the full implications of future earings
...
Consider the behavior of stock prices in a market where earnngs expectations fail to reflect the pattern described in Table 1. Use a seasonal random walk with trend. 
...
The abnormal retrun reflects a compnent equal to the current detrended seasonal difference in earnings. 
...
__Claim:__ If Hypothesis 1 is correct, then we should observe a positive relation for each of the next three announcements but with the magnitude of the assoication decling over the three quarters and a negative relation for the fourth subsequent announcement. 
...
__Result:__ Three-day abnormal returns around earnings announcements are predictableas seen in Panel A of _Table 2_. Signs and magnitudes lend support of Hypothesis 1. 
...
__Result:__ Panel C shows relation between SUE deciles for quater $t$ and corresponding mean abnormal returns around next four announcements is monotonic and almost linear. 
...
__Claim:__ Data are consistent with stock prices faiing to reflect fully the implications of current earnings for future earnings for all firms, but the faiure is more apparent for small stocks. 
...
__Note:__ Sample is too short to iteratively estimate the time series model here. To deal with this, they first partition the data into a subset of 26 historical observations, and they also use the full sample but subsitute forecast errors from alternative Foster model. This weakens the test. 
...
(1) _Can this evidence be explained in terms of rational investors desire to await confirmation that a previous earnings change is not transitory?_ No, not consistent with quater $t+4$. 
