...
### Big Question: Does risk affect the value premium and if so, how?
...
- Value minus growth betas tend to covary positively with expected market risk premium
...
(1) Sorting 
(2) Beta-Premium Regression (Conditional CAPM)
...
Theory suggests that expected market risk premium is countercyclical
...
Define $\gamma_t$ as the expected market risk premium. Both $\beta_{it}$ and $\gamma_t$ are conditional on the information st at time $t$. Then, the conditional CAPM says: $\mathbb{E}[r_{it+1}] = \gamma_t \beta_{it}$.
...
- Stocks with positive beta-premium sensitivities have high risk precisely during the recessionary periods when investors dislike risk or when price of risk is high
...
$$\textbf{Z}_t = [\textbf{1}, DIV_tDEF_tTERMtTB_t]$$
...
- Value prefmium exists in the long run, especially among small stocks
...
Exluding Great Depression drastically affects small growth stocks
...
- Casts doubt on claim that value cannot be riskier than growth
